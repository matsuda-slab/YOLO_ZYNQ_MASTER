...
void set_yolo_conv(u32 output_ch, u32 input_ch, u32 fold_output_ch, u32 fold_input_ch,
    ...
    u32 fold_win_area)
{
  /* yolo_conv_top というモジュールに, それぞれのパラメータをわたしてる */
  /* yolo_conv_top は たぶんhlsで書いてあるモジュール */
...
void forward_layer_group(layer_group l)
{
  ...
  for(int i = 0; i < l.output_fold_factor; i++)
  {
    ...
    // jがinput_fold_factor-1 になったときが, 入力チャネル数すべてを処理したとき
    for(int j = 0; j < l.input_fold_factor; j++)
    {
      ...
      /* グループの最後でない(入力チャネル数分の処理が終わってない)場合は, 値を返さずにそのままIP間で処理する */
      /* バイアス, 入力値の順で, DMAでPLに送る */
      if(j==l.input_fold_factor-1)
      {
        if(!l.conv_disable)
        {
...
int main()
{
  ...
  layer_group group_4 = make_layer_group(26,26,32,4,            // in_c  = 128
      ...
      0,LEAKY,NONE,2);
  layer_group group_5 = make_layer_group(26,26,32,1,            // in_c  = 256 のはずだが, input_fold_factor=1 ?
  ...
  while (1) {
    ...
    // 座標を元に, 矩形を描画する
    //XGpioPs_WritePin(&instXGpioPs, 7, 0);     // 一旦検出フラグ(LED)を0にする

    // SPIに(PSPLのレジスタに)検出数を出力する
    XTime_GetTime(&tStart);
    ...
    for(int idx = 0; idx < point_list.size(); idx++) {
      /* Write Low to MIO7 */
      //XGpioPs_WritePin(&instXGpioPs, 7, 1);   // 1個でも検出したら, LEDを1にする
