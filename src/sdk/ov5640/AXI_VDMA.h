...
public:
	...
	AXI_VDMA(uint16_t dev_id, uint32_t frame_buf_base_addr_w, uint32_t frame_buf_base_addr_r, IrptCtl& irpt_ctl, uint16_t rd_irpt_id, uint16_t wr_irpt_id) :
		...
		context_{},
		frame_buf_base_addr_w_(frame_buf_base_addr_w),
		frame_buf_base_addr_r_(frame_buf_base_addr_r),
		irpt_ctl_(irpt_ctl)
	{
...
//		while (XAxiVdma_ChannelIsRunning(&drv_inst_.WriteChannel)) ;
	...
	void configureRead(uint16_t h_res, uint16_t v_res)
	{
		...
		context_.ReadCfg.FixedFrameStoreAddr = 1; //park it on 0 until we sync
		status = XAxiVdma_DmaConfig(&drv_inst_, XAXIVDMA_READ, &context_.ReadCfg);

    XAxiVdma_StartParking(&drv_inst_, 0, XAXIVDMA_READ);

		if (XST_SUCCESS != status)
		{
			throw std::runtime_error(__FILE__ ":" LINE_STRING);
		}
		uint32_t addr = frame_buf_base_addr_r_;
		for (int iFrm=0; iFrm<drv_inst_.MaxNumFrames; ++iFrm) {
			context_.ReadCfg.FrameStoreStartAddr[iFrm] = addr;
			xil_printf("VDMA Frame %d Addr (READ): 0x%08x\r\n", iFrm, addr);
	...
	void configureWrite(uint16_t h_res, uint16_t v_res)
	{
		...
		context_.WriteCfg.FixedFrameStoreAddr = 1; //ignored, since we circle through buffers
		status = XAxiVdma_DmaConfig(&drv_inst_, XAXIVDMA_WRITE, &context_.WriteCfg);

    XAxiVdma_StartParking(&drv_inst_, 0, XAXIVDMA_WRITE);

		if (XST_SUCCESS != status)
		{
			throw std::runtime_error(__FILE__ ":" LINE_STRING);
		}
		uint32_t addr = frame_buf_base_addr_w_;
		for (int iFrm=0; iFrm<drv_inst_.MaxNumFrames; ++iFrm) {
			context_.WriteCfg.FrameStoreStartAddr[iFrm] = addr;
			xil_printf("VDMA Frame %d Addr (WRITE): 0x%08x\r\n", iFrm, addr);
  ...
  int get_buffer_addr_r() {
    return frame_buf_base_addr_r_;
  }

  int get_buffer_addr_w() {
    return frame_buf_base_addr_w_;
  }

  void get_pixels(bgr_t* img) {
    u8* ptr = (u8*)frame_buf_base_addr_r_;
    for(int i = 0; i < 1920*1080*3; i = i + 3) {
      img[i/3].r = ptr[i];
      img[i/3].g = ptr[i+1];
      img[i/3].b = ptr[i+2];
    }
...
private:
  ...
  uint32_t frame_buf_base_addr_w_;
  uint32_t frame_buf_base_addr_r_;
	IrptCtl& irpt_ctl_;
